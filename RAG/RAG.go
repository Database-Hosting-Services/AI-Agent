...
type RAGmodel interface {
	...
	QueryChatMultiNamespace(namespaces []string, query string, topK int) (string, []string, error)
...
// QueryChatMultiNamespace performs a chat query across multiple namespaces, combining the results
// If namespaces is nil or empty, it will use the default namespace
// This function searches each namespace separately and combines the results
func (r *RAGPineconeGemini) QueryChatMultiNamespace(namespaces []string, query string, topK int) (string, []string, error) {
	if topK == 0 {
		topK = DEFAULT_TOP_K
	}
	...
	if len(namespaces) == 0 {
		defaultNamespace := "database-articles"
		log.Printf("INFO: No namespaces specified, using default: %s", defaultNamespace)
		return r.QueryChat(defaultNamespace, query, topK)
	}

	log.Printf("Processing multi-namespace chatbot query across %d namespaces", len(namespaces))

	// Set up collection for combined results
	combinedResources := ""
	combinedSources := []string{}
	sourceMap := make(map[string]bool)

	// Calculate topK per namespace to maintain approximately the same total
	nsTopK := topK / len(namespaces)
	if nsTopK < 1 {
		nsTopK = 1
	}

	// Create a wait group to process namespaces concurrently
	var wg sync.WaitGroup
	var mu sync.Mutex // For synchronizing access to combinedResources and combinedSources
	...
	for _, ns := range namespaces {
		...
		go func(namespace string) {
			...
			log.Printf("Searching namespace: %s", namespace)
			matches, err := r.Match(namespace, query, nsTopK)
			if err != nil {
				log.Printf("WARNING: Error searching namespace %s: %v", namespace, err)
				return
			}

			if len(matches) == 0 {
				log.Printf("INFO: No matches found in namespace %s", namespace)
				return
			}

			// Process matches for this namespace
			nsResources := fmt.Sprintf("--- Results from namespace: %s ---\n", namespace)
			nsSources := []string{}
			...
			for _, match := range matches {
				...
				if url, ok := match.Vector.Metadata.Fields["source_url"]; ok {
					...
					if !sourceMap[sourceURL] {
						sourceMap[sourceURL] = true
						nsSources = append(nsSources, sourceURL)
						nsResources += fmt.Sprintf("Source: %s\n\n", sourceURL)
					}
					mu.Unlock()
				}
				...
				if content, ok := match.Vector.Metadata.Fields["content"]; ok {
					contentText := strings.Trim(content.GetStringValue(), "\"\n \t")
					if contentText != "" {
						nsResources += contentText + "\n"
					} else {
						log.Printf("WARNING: Empty content found in namespace %s, match ID: %s",
							namespace, match.Vector.Id)
					}
				} else {
					log.Printf("WARNING: No 'content' field in metadata for namespace %s, match ID: %s",
						namespace, match.Vector.Id)

					// Fallback: try other metadata fields
					for key, value := range match.Vector.Metadata.Fields {
						if key != "source_url" && value.GetStringValue() != "" {
							nsResources += fmt.Sprintf("%s: %s\n", key, value.GetStringValue())
						}
					}
				}
			}
			...
			// Add this namespace's results to the combined results
			mu.Lock()
			combinedResources += nsResources
			combinedSources = append(combinedSources, nsSources...)
			mu.Unlock()

		}(ns)
	}

	// Wait for all namespace searches to complete
	wg.Wait()

	// Check if we found any results
	if combinedResources == "" {
		log.Printf("WARNING: No matches found in any namespace")
		return "I don't have specific information about that in any of my knowledge sources. Could you rephrase your question?", nil, nil
	}

	// Format the prompt using the chatbot template
	prompt := fmt.Sprintf(CHATBOT_PROMPT_TEMPLATE, combinedResources, query)
	...
	log.Printf("Successfully generated response from %d namespaces with %d unique sources",
		len(namespaces), len(combinedSources))

	return responseText, combinedSources, nil
}
